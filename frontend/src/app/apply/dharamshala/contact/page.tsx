  ...
  const handleSendOTP = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        window.location.href = `/apply/dharamshala/verify?token=${encodeURIComponent(data.token)}`;
      } else {
    ...
    <div className="min-h-screen" style={{ background: "var(--bg-page)" }}>
      ...
      <main className="px-6 py-12">
        <div className="mx-auto max-w-2xl">
          ...
          <div className="flex items-start gap-3 mb-8 p-4 rounded-lg" style={{ backgroundColor: "var(--color-amber-50)" }}>
          ...
          {!otpSent ? (
            ...
            <div className="space-y-4">
              <div className="card p-4 text-center">
                ...
                {resendTimer > 0 ? (
                  <div className="flex items-center gap-2">
                    <Clock className="w-4 h-4" style={{ color: "var(--color-amber-600)" }} />
